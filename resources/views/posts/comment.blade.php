...
<!-- Comment -->
			...
			<!-- Show Comment -->
				...
				<div class="w3-container w3-padding-4 comment">
					...
					@if(Auth::check())
						...
						<a href="{{ route('comments.destroy', ['id'=>$comment->id]) }}" onclick="deleteComment(event,$(this));" data-token=" {{ csrf_token() }}" data-id="{{$comment->id}}" class="w3-right"><i class="fa fa-trash-o "></i>Delete</a>
						@endif
					@endif
				</div>
				@endforeach
